...
HYDRA_FULL_ERROR=1 torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:0 --nnodes=1 --nproc_per_node=4 pangaea/run.py  --config-name=train  dataset=agbd  encoder=croma_joint  decoder=reg_upernet  preprocessing=reg_resize  criterion=mse  task=regression finetune=true lora=default batch_size=8 num_workers=6 test_num_workers=6 test_batch_size=32 use_wandb=True task.trainer.eval_interval=0.25 dataset.root_path_cluster=${TMPDIR} task.trainer.log_interval=100
