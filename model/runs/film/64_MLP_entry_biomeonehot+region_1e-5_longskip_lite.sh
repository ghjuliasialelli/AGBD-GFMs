...
lite="true"
lite_eval_big="true" # whether to evaluate on the big AGBD dataset even when training on the lite version
...
if [ "$first_part" == "cluster" ]; then
    ...
    if [ "$lite" == "true" ]; then
        rsync -aq /cluster/work/igp_psr/gsialelli/Data/patches/AGBD-Lite ${TMPDIR}
        rsync -aq /cluster/work/igp_psr/gsialelli/EcosystemAnalysis/Models/Baseline/cat2vec/AGBD-Lite/embeddings_train.csv ${TMPDIR}
    fi
    if [ "$lite" == "true" ] && [ "$lite_eval_big" == "true" ] || [ "$lite" == "false" ]; then
        ...
        rsync -aq /cluster/work/igp_psr/gsialelli/Data/patches/statistics_subset_2019-2020-v4-1.pkl ${TMPDIR}
    fi
    ...
    rsync -aq /cluster/work/igp_psr/gsialelli/EcosystemAnalysis/Models/Baseline/cat2vec/AGBD/embeddings_train.csv ${TMPDIR}

    # Move the .pkl file with the AGB residuals statistics
    rsync -aq /cluster/work/igp_psr/gsialelli/Data/patches/nico_film_17997535-1_17997535-2_17997535-3_train_agb_residuals_stats.pkl ${TMPDIR}
