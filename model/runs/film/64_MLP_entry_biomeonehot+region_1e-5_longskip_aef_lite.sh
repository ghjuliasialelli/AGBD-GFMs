...
if [ "$first_part" == "cluster" ]; then
    ...
    if [ "$tmpdir" == "true" ]; then
        ...
        if [ "$lite" == "true" ]; then
            rclone copy /cluster/work/igp_psr/gsialelli/Data/patches/AGBD-Lite/ ${TMPDIR} --include "*.h5" --include "AGBD-Lite-statistics.pkl" --transfers 16 --checkers 32
        ...
        if [ "$lite" == "true" ] && [ "$lite_eval_big" == "true" ] || [ "$lite" == "false" ]; then
            rclone copy /cluster/work/igp_psr/gsialelli/Data/patches/ ${TMPDIR} --include "*v4_*-20.h5" --include "statistics_subset_2019-2020-v4*.pkl" --transfers 16 --checkers 32
        fi
        if [ "$aef" == "true" ]; then
        ...
        cp /cluster/work/igp_psr/gsialelli/Data/patches/nico_film_17997535-1_17997535-2_17997535-3_train_agb_residuals_stats.pkl ${TMPDIR}

        # Move the .pkl that maps the idx from the -Lite dataset to the original dataset
