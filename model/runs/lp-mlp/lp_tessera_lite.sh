...
tmpdir="true"
# if true, put back #SBATCH --tmp=300G
...
if [ "$first_part" == "cluster" ]; then
    ...
    if [ "$tmpdir" == "true" ]; then
        ...
        if [ "$tessera" == "true" ]; then
            rclone copy /cluster/work/igp_psr/gsialelli/Data/patches/TESSERA/ ${TMPDIR} --include "TESSERA-Lite-*.h5" --include "*statistics*.pkl" --transfers 16 --checkers 32
        fi
