"""
Generate the 15 ablation .sh scripts in this directory.

Matrix:
    source axis: aef, agbd, aef_agbd*    (input modalities)
    scope axis:  without, only, 2019, 2020_subset, 2019_2020_subset
...
SOURCES = ['aef', 'agbd', 'aef_agbd*']
SCOPES  = ['without', 'only', '2019', '2020_subset', '2019_2020_subset']

# --- source axis: input modality toggles --------------------------------------
SOURCE_CFG = {
    'aef': dict(
        aef='true',
        bands_arr='()',
        s2_dates='false', s2_day='false', s2_doy='false',
        alos='false',
        lc='false', ft_cat2vec='false',
        dem='false', topo='false', aspect='false', slope='false',
    ),
    'agbd': dict(
        aef='false',
        bands_arr='(B01 B02 B03 B04 B05 B06 B07 B08 B8A B09 B11 B12)',
        s2_dates='true', s2_day='true', s2_doy='true',
        alos='true',
        lc='true', ft_cat2vec='true',
        dem='true', topo='true', aspect='true', slope='true',
    ),
    'aef_agbd*': dict(
        aef='true',
        bands_arr='()',
        ...
        alos='false',
        lc='true', ft_cat2vec='true',
        dem='false', topo='true', aspect='true', slope='true',
    ),
}

# --- scope axis: ablation-regime toggles --------------------------------------
SCOPE_CFG = {
    'without': dict(
        array='1-3', log_suffix='-%a',
        years_arr='(2019 2020)',
        geo_ablation='true', keep_region='false',
        subsample_2020='false', years_stats='None',
    ),
    'only': dict(
        array='1-3', log_suffix='-%a',
        years_arr='(2019 2020)',
        geo_ablation='true', keep_region='true',
        subsample_2020='false', years_stats='None',
    ),
    '2019': dict(
        array='1-1', log_suffix='',
        years_arr='(2019)',
        geo_ablation='false', keep_region='false',
        subsample_2020='false', years_stats='None',
    ),
    '2020_subset': dict(
        array='1-1', log_suffix='',
        years_arr='(2020)',
        geo_ablation='false', keep_region='false',
        subsample_2020='true', years_stats='2020',
    ),
    '2019_2020_subset': dict(
        array='1-1', log_suffix='',
        years_arr='(2019 2020)',
        geo_ablation='false', keep_region='false',
        subsample_2020='true', years_stats='2019-2020',
    ),
}


def build_script(source: str, scope: str) -> str:
    s, sc = SOURCE_CFG[source], SCOPE_CFG[scope]
    # latlon is on when we have S2 input (i.e. source != aef) and we're in a
    # temporal-ablation scope, off otherwise.
    latlon = 'true' if (source != 'aef' and scope in {'2019', '2020_subset', '2019_2020_subset'}) else 'false'
    ...
    return dedent(f"""\
        ...
        #SBATCH --output=/cluster/scratch/gsialelli/logs/training-%A{sc['log_suffix']}.txt
        #SBATCH --error=/cluster/scratch/gsialelli/logs/training-%A{sc['log_suffix']}.txt
        #SBATCH --mem-per-cpu=8G
        #SBATCH --job-name=models
        #SBATCH --array={sc['array']}
        ...
        years={sc['years_arr']}
        subsample_2020="{sc['subsample_2020']}"
        years_stats="{sc['years_stats']}"
        ...
        geo_ablation="{sc['geo_ablation']}"
        keep_region="{sc['keep_region']}"
...
def main():
    for source in SOURCES:
        for scope in SCOPES:
            name = f"{source}_{scope}.sh"
            (HERE / name).write_text(build_script(source, scope))
            print(f"wrote {name}")
