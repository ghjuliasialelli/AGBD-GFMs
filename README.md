...
# then set your paths (see Configuration below)
$EDITOR config.sh
```
...
**Paths not covered by `config.sh`.** Some absolute paths predate it and must still be updated
by hand: the `cp`/`rclone` lines in the `.sh` launchers stage inputs into `$TMPDIR` from the
authors' cluster paths (`/cluster/work/igp_psr/gsialelli/...`), which you cannot read, and the
data-prep and analysis scripts hard-code a few of their own (e.g. `data/aef/`,
`comparison/agbref/comparison.py`). Point them at your own copies before running.
...
> `config.sh` holds only the entity and project (the entity defaults to `gs-tp-biomass` and can
> be overridden with `WANDB_ENTITY`). wandb reads the key itself from `~/.netrc` or
> `$WANDB_API_KEY`.
